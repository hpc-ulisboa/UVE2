#!/usr/bin/env python3

import os
import re
import glob
import sys
import subprocess

def is_executable(filepath):
    """Check if the file is executable."""
    return os.path.isfile(filepath) and os.access(filepath, os.X_OK)

# Directories
input_dir = 'benchmarks'      # Path to input directory

# Desired output file extension (e.g., .din for DineroIV)
output_extension = '.din'

# Fetch all subdirectories from input_dir
subdirs = [f.path for f in os.scandir(input_dir) if f.is_dir()]

for root, dirs, files in os.walk(input_dir):
    for file in files:
        filepath = os.path.join(root, file)
        
        if is_executable(filepath):
            print(f"Getting trace from : {filepath}...")

            input_filename = os.path.basename(filepath) # Get the filename without directory
            output_filepath = os.path.join(root, input_filename.split('.')[1]+output_extension)  # Output file path

            with open(output_filepath, 'w') as output_file:

                process = subprocess.Popen(
                    ['../spike', '-l', '--log-commits', '../pk', filepath],  # Command split into a list
                    stdout=subprocess.DEVNULL,  # Capture the output
                    stderr=subprocess.PIPE,  # Ignore errors if needed, or log separately
                    bufsize=1,  # Line buffering
                    universal_newlines=True  # Ensure text output, not bytes
                )

                # Read input directly form process stderr
                trace_file = process.stderr

                # Open the Spike log file for reading and the DineroIV trace file for writing
                collecting = False

                with open(output_filepath, 'w') as output_file:

                    prev_line = None  # To keep track of the previous line (instruction line)
                    for line in trace_file:
                        if 'instret' in line:
                            if not collecting:
                                # First instret line, start collecting
                                collecting = True
                            else:
                                # Second instret line, stop collecting
                                break
                        if collecting:
                            # Search for lines with memory accesses
                            if prev_line and 'mem' in line:
                                instruction = prev_line.split()[4].strip()
                                num = 0
                                # Load instructions (reads) have the format mem 0x<address>
                                # Store instructions (writes) have the format mem 0x<address> <value>
                                # One line can have multiple memory accesses and a mix of reads and writes
                                accesses = line.split('mem')
                                for access in accesses[1:]:
                                    # Split the access into words
                                    access = access.strip().split() 
                                    # Extract the memory address
                                    address_match = re.search(r'0x[0-9a-fA-F]+', access[0])
                                    if address_match:
                                        address = address_match.group(0) 
                                        if len(access) < 2:
                                            # Load instruction (read) is a 0 in the dinero trace file
                                            output_file.write(f"0 {address} {instruction} ({num})\n")
                                        else:
                                            # Store instruction (write) is a 1 in the dinero trace file
                                            output_file.write(f"1 {address} {instruction} ({num})\n")
                                        num += 1

                        # Store the current line as the previous line for the next iteration
                        prev_line = line