...
std::visit(overloaded{
    [&](StreamReg8 &src) { uint8_t value = READ_REG(destReg); baseBehaviour(destReg, src, predReg, (unsigned char){}); WRITE_REG(destReg, value); },
    [&](StreamReg16 &src) { uint16_t value = READ_REG(destReg); baseBehaviour(destReg, src, predReg, (unsigned short int){}); WRITE_REG(destReg, value); },
    [&](StreamReg32 &src) { uint32_t value = READ_REG(destReg); baseBehaviour(destReg, src, predReg, (unsigned int){}); WRITE_REG(destReg, value); },
    [&](StreamReg64 &src) { uint64_t value = READ_REG(destReg); baseBehaviour(destReg, src, predReg, (unsigned long int){}); WRITE_REG(destReg, value); },
