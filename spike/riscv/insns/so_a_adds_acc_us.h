...
std::visit(overloaded{
    [&](StreamReg8 &src) { unsigned char value = value = readRegAS(unsigned char, destReg); baseBehaviour(destReg, src, predReg, (unsigned char){}); WRITE_REG(destReg, value); },
    [&](StreamReg16 &src) { unsigned short int value = value = readRegAS(unsigned short int, destReg); baseBehaviour(destReg, src, predReg, (unsigned short int){}); WRITE_REG(destReg, value); },
    [&](StreamReg32 &src) { unsigned int value = value = readRegAS(unsigned int, destReg); baseBehaviour(destReg, src, predReg, (unsigned int){}); WRITE_REG(destReg, value); },
    [&](StreamReg64 &src) { unsigned long int value = value = readRegAS(unsigned long int, destReg); baseBehaviour(destReg, src, predReg, (unsigned long int){}); WRITE_REG(destReg, value); },
