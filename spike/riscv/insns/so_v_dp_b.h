...
const char value = READ_REG(baseReg);
...
auto baseBehaviour = [](auto &dest, auto &pred, const auto value) {
    ...
    std::vector<uint8_t> out(destValidIndex, readAS<uint8_t>(value));
    for (size_t i = 0; i < destValidIndex; ++i)
        out.at(i) = p.at((i+1)*sizeof(uint64_t)-1) ? readAS<uint8_t>(value) : destElements.at(i);
